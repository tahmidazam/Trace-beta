...
struct CompressedTraceDocumentContents: Codable {
    ...
    init(from contents: TraceDocumentContents) {
        ...
        eventTypes = contents.eventTypes
        events = Event.compress(events: contents.events, eventTypes: eventTypes)
        ...
        potentialRange = contents.potentialRange
    }
    ...
    func uncompressed() -> TraceDocumentContents {
        ...
        let uncompressedEvents: [Event] = Array(eventTypes.compactMap { eventType in
            if let sampleIndices = events[eventType] {
                return sampleIndices.map({ sampleIndex in
                    return Event(sampleIndex: sampleIndex, type: eventType)
                })
            } else {
                return nil
            }
        ...
        return TraceDocumentContents(subject: subject, info: info, streams: uncompressedStreams, sampleRate: sampleRate, events: uncompressedEvents, eventTypes: eventTypes, epochLength: epochLength, potentialRange: potentialRange)
    }
}
